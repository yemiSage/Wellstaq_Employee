...
function ChallengesTab() {
  ...
  return <>
    ...
    <div className="page-pad !pt-0 list-stack"><QueryState query={query}>{query.data?.items.length ? query.data.items.map((c) => <Link className="card-link" key={c.id} to={`/challenges/${c.id}`}><Card className="content-card activity-challenge-card rounded-2xl"><img className="h-40 w-full rounded-2xl object-cover" src={c.image_url ?? "/assets/figma/event-mindful.jpeg"} alt="" /><span className="chip">{titleCase(c.status)}</span><h2>{c.name}</h2><p>{c.description}</p><small>{c.participant_count} participants · {c.end_date ? `Until ${formatDate(c.end_date)}` : "Open challenge"}</small></Card></Link>) : <EmptyState title={`No ${challengeFilters.find((entry) => entry.key === filter)!.label.toLowerCase()} challenges`} body="Challenges from your organization will appear here." />}<Pagination offset={offset} total={query.data?.total ?? 0} onChange={setOffset} /></QueryState></div>
  </>;
}
