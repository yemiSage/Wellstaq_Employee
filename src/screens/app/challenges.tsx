...
export function CreateChallengeScreen() {
  ...
  const save=useMutation({mutationFn:async()=>{const image_url=file?await employeeApi.upload(file,"challenges"):null;return employeeApi.createChallenge(user!.organizationId,{...values,branch_id:user!.branchId,target_value:Number(values.target_value),image_url});},onSuccess:(c)=>{void client.invalidateQueries({queryKey:["challenges"]});navigate(`/challenges/${c.id}`,{replace:true});}});
  const ready = values.name.trim() && values.start_date && values.end_date;
  return <Screen title="Create challenge"><h1 className="page-title">A shared goal.</h1><form className="list-stack" onSubmit={(e)=>{e.preventDefault();save.mutate();}}><CoverImageField file={file} onChange={setFile} hint="Optional. Images up to 20 MB." />{([['name','Challenge name','text'],['start_date','Start date','date'],['end_date','End date','date'],['metric_type','Metric (e.g. steps)','text'],['target_value','Target','number']] as const).map(([key,label,type])=><Field key={key} label={label}><Input type={type} required min={type==='number'?'1':key==='end_date'?values.start_date:undefined} value={values[key]} onChange={(e)=>setValues({...values,[key]:e.target.value})}/></Field>)}<Field label="Description"><textarea className="textarea" rows={3} value={values.description} onChange={(e)=>setValues({...values,description:e.target.value})}/></Field><Field label="Goal type"><select className="select" value={values.target_type} onChange={(e)=>setValues({...values,target_type:e.target.value})}><option value="cumulative">Cumulative</option><option value="daily_minimum">Daily minimum</option></select></Field><FormError error={save.error}/><Button disabled={!ready} loading={save.isPending}>Create challenge</Button></form></Screen>;
}
