...
export function ExploreScreen() {
  ...
  return <div><PageHeader title="Explore"/><div className="page-pad list-stack"><div className="section-intro"><p className="eyebrow">Your community</p><h1 className="page-title">A little more<br/>connected.</h1></div><div className="flex flex-wrap gap-4"><Link className="text-link" to="/posts/new">Share a moment +</Link><Link className="text-link" to="/stories/new">Add a story +</Link></div><QueryState query={stories}>{Boolean(stories.data?.items.length)&&<div className="story-rail">{stories.data?.items.map((story)=><Link className="story" key={story.id} to={`/stories/${story.id}`}><span>{story.media_type==='image'?<img src={story.media_url} alt=""/>:<span>▶</span>}</span><small>{story.user_id===user?.id?"Your story":"Colleague"}</small></Link>)}</div>}</QueryState><div className="grid grid-cols-2 gap-3"><Link className="card-link" to="/challenges"><Card className="content-card"><span className="eyebrow">Grow together</span><h2>Challenges ↗</h2></Card></Link><Link className="card-link" to="/clubs"><Card className="content-card"><span className="eyebrow">Find your people</span><h2>Clubs ↗</h2></Card></Link></div><div className="segmented"><button className={!saved?'active':''} onClick={()=>{setSaved(false);setOffset(0);}}>Community</button><button className={saved?'active':''} onClick={()=>{setSaved(true);setOffset(0);}}>Saved posts</button></div><QueryState query={query}>{query.data?.items.length?query.data.items.map((post)=><PostCard key={post.id} post={post} saved={saved}/>):<EmptyState title={saved?"Keep the good things close":"Your community is warming up"} body={saved?"Save a post to find it here later.":"Be the first to share a moment with your team."}/>}<Pagination offset={offset} total={query.data?.total??0} onChange={setOffset}/></QueryState></div></div>;
}
