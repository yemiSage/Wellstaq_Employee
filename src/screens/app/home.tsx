...
export function HomeScreen() {
  ...
  return <div className="home-page" ref={rootRef}><header className={`home-header${scrolled ? " is-scrolled" : ""}`}><button className="home-identity" onClick={() => navigate("/profile")} aria-label={`Open profile. ${points.data?.total ?? 0} points`}><span className="avatar !rounded-full">{user?.avatarUrl ? <img src={user.avatarUrl} alt="" /> : initials(user?.firstName, user?.lastName)}</span><span className="home-identity-score" aria-hidden="true"><Cup size="17" color="currentColor" /><strong>{points.data?.total ?? 0}pts</strong></span></button><button className="header-icon" onClick={() => navigate("/notifications")} aria-label="Notifications"><Notification color="currentColor" size="23" />{data.unreadCount > 0 && <span className="notification-dot">{Math.min(data.unreadCount, 9)}</span>}</button></header>
    ...
    <section className="wellbeing-ribbon"><div className="ribbon-copy"><span className="eyebrow">Your daily check-in</span><strong>{checked ? "Well done." : "How are you?"}</strong></div><span className="ribbon-mood wellbeing-orb" aria-hidden="true"/><Link className="ribbon-action" to="/checkin"><span>{checked ? "View today’s check-in" : "Start today’s check-in"}</span><ArrowRight2 size="20" color="currentColor" /></Link></section>
    <section className="section-block stat-grid"><Card><span className="stat-icon-checkin"><HeartTick size="21" color="currentColor" /></span><strong>{checked ? "Done" : "Today"}</strong><small>Your personal check-in</small></Card><Card><span className="stat-icon-events"><Calendar size="21" color="currentColor" /></span><strong>{data.events.length}</strong><small>Upcoming events</small></Card></section>
