...
export function HomeScreen() {
  ...
  return <div className="home-page" ref={rootRef}><header className={`home-header${scrolled ? " is-scrolled" : ""}`}><button className="avatar !rounded-full" onClick={() => navigate("/profile")} aria-label="Open profile">{user?.avatarUrl ? <img src={user.avatarUrl} alt="" /> : initials(user?.firstName, user?.lastName)}</button><button className="header-icon" onClick={() => navigate("/notifications")} aria-label="Notifications"><Notification color="currentColor" size="23" />{data.unreadCount > 0 && <span className="notification-dot">{Math.min(data.unreadCount, 9)}</span>}</button></header>
    ...
    <div className="home-summary"><Link to="/leaderboard" aria-label={`${streak}-day check-in streak. Open the leaderboard`}><span aria-hidden="true"><Cup size="19" color="currentColor" /></span><strong>{streak}-day streak</strong></Link></div>
    ...
    <section className="section-block section-rail"><div className="section-heading"><h2>Featured events</h2><Link className="text-link" to="/events">See more</Link></div>{data.events.length ? <div className="event-rail">{data.events.map((event) => <Link className="event-rail-card" key={event.id} to={`/events/${event.id}`}><img src={event.imageUrl ?? "/assets/figma/event-mindful.jpeg"} alt="" /><div className="event-rail-body">{event.participantCount > 0 && <span className="event-rail-chip">{event.participantCount} going</span>}<strong>{event.title}</strong><div className="event-rail-meta"><Calendar size="16" color="currentColor" /><span>{formatDate(event.startDate)}</span>{event.time && <><span className="event-rail-dot" aria-hidden="true" /><span>{event.time}</span></>}</div></div></Link>)}</div> : <p className="empty-note">No upcoming events yet. Your next shared experience will appear here.</p>}</section></div>;
}
