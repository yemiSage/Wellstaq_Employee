...
export function InviteScreen() {
  ...
  if (step === 0) return <AuthLayout><Link className="auth-back" to="/login"><ArrowLeft2 color="currentColor" size="18" />Go back</Link><div className="auth-copy"><h1>Let’s find your workspace.</h1><p>Use the invite code shared by your People team.</p></div><form className="grid gap-5" onSubmit={codeForm.handleSubmit(async ({ inviteCode: value }) => { await authApi.requestInviteOtp(value); setInviteCode(value); setStep(1); })}><div className="invite-code-field"><Field label="Invite code" error={codeForm.formState.errors.inviteCode?.message}><Input autoCapitalize="characters" {...codeForm.register("inviteCode")} /></Field></div><Button disabled={!codeForm.formState.isValid || codeForm.formState.isSubmitting}>Confirm code</Button></form></AuthLayout>;
