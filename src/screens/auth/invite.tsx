...
export function InviteScreen() {
  ...
  const codeForm = useForm<{ inviteCode: string }>({ resolver: zodResolver(z.object({ inviteCode: z.string().min(2) })), defaultValues: { inviteCode: "" } });
  const otpForm = useForm<{ code: string }>({ resolver: zodResolver(z.object({ code: z.string().length(6) })), defaultValues: { code: "" } });
  const detailsForm = useForm<Details>({ resolver: zodResolver(z.object({ firstName: z.string().min(1), lastName: z.string().min(1), password: z.string().min(8), country: z.string().min(2), state: z.string().min(1) })), defaultValues: { firstName: "", lastName: "", password: "", country: "Nigeria", state: "Lagos" } });

  if (step === 0) return <AuthLayout><div className="auth-copy"><p className="eyebrow">Employee invitation</p><h1>Let’s find your workspace.</h1><p>Use the invite code shared by your People team.</p></div><form className="grid gap-5" onSubmit={codeForm.handleSubmit(async ({ inviteCode: value }) => { await authApi.requestInviteOtp(value); setInviteCode(value); setStep(1); })}><Field label="Invite code" error={codeForm.formState.errors.inviteCode?.message}><Input autoCapitalize="characters" {...codeForm.register("inviteCode")} /></Field><Button>Send verification code</Button></form></AuthLayout>;
  if (step === 1) return <AuthLayout><div className="auth-copy"><p className="eyebrow">Verify invitation</p><h1>Check your email.</h1><p>Enter the 6-digit code we sent to the invited address.</p></div><form className="grid gap-5" onSubmit={otpForm.handleSubmit(async ({ code }) => { const response = await authApi.verifyInviteOtp(inviteCode, code); setVerificationToken(String(response.invite_verification_token ?? response.verification_token ?? "")); setStep(2); })}><Field label="Verification code" error={otpForm.formState.errors.code?.message}><Input className="otp-input" inputMode="numeric" maxLength={6} {...otpForm.register("code")} /></Field><Button>Verify invitation</Button></form></AuthLayout>;
  if (step === 2) return <AuthLayout><div className="auth-copy"><p className="eyebrow">Your details</p><h1>Make it yours.</h1><p>This information keeps your account personal and secure.</p></div><form className="grid grid-cols-2 gap-4" onSubmit={detailsForm.handleSubmit((values) => { setDetails(values); setStep(3); })}><Field label="First name" error={detailsForm.formState.errors.firstName?.message}><Input autoComplete="given-name" {...detailsForm.register("firstName")} /></Field><Field label="Last name" error={detailsForm.formState.errors.lastName?.message}><Input autoComplete="family-name" {...detailsForm.register("lastName")} /></Field><div className="col-span-2"><Field label="Password" error={detailsForm.formState.errors.password?.message}><Input type="password" autoComplete="new-password" {...detailsForm.register("password")} /></Field></div><Field label="Country"><Input {...detailsForm.register("country")} /></Field><Field label="State"><Input {...detailsForm.register("state")} /></Field><Button className="col-span-2">Continue to wellbeing</Button></form></AuthLayout>;
  if (step === 3) {
