...
export function InviteScreen() {
  ...
  return <AuthLayout hideIntro onBack={() => step ? setStep(step - 1) : navigate("/login")} progress={{ segments: 5, active: macroStage, fraction: macroFraction }}>
    ...
    {step === 2 && <><div className="auth-copy"><h1>Make it yours.</h1><p>A few details to create your account.</p></div><form className="grid grid-cols-2 gap-4" onSubmit={(e) => { e.preventDefault(); setStep(3); }}>{([['first_name','First name'],['last_name','Last name'],['country','Country'],['state','State']] as const).map(([key,label]) => <Field label={label} key={key}><Input value={details[key]} required minLength={key === 'country' ? 2 : 1} maxLength={100} onChange={(e) => setDetails({ ...details, [key]: e.target.value })} /></Field>)}<div className="col-span-2"><Field label="Password" hint="At least 8 characters"><Input type="password" autoComplete="new-password" minLength={8} required value={details.password} onChange={(e) => setDetails({ ...details, password: e.target.value })} /></Field></div><Button className="col-span-2" disabled={!details.first_name.trim() || !details.last_name.trim() || !details.country.trim() || !details.state.trim() || details.password.length < 8}>Next</Button></form></>}
