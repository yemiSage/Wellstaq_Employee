...
export function ForgotPasswordScreen() {
  ...
  const form = useForm<{ email: string }>({ resolver: zodResolver(z.object({ email: z.string().email() })), defaultValues: { email: "" } });
...
export function ResetPasswordScreen() {
  ...
  const form = useForm<{ password: string }>({ resolver: zodResolver(z.object({ password: z.string().min(8) })), defaultValues: { password: "" } });
